...
## Define agent portrayal (color, size, shape) based on behavior_type
def agent_portrayal(agent):
    ...
    if agent.behavior_type not in color_map:
        # Return an empty dict to avoid breaking rendering
        return {}

    return {
        "marker": "o",
        "color": color_map[agent.behavior_type],
        "size": 20,
    }
